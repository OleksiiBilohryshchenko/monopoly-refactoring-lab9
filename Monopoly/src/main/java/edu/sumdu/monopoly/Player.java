...
public class Player {
	...
	public void exchangeProperty(Player player) {
		for (int i = 0; i < getPropertyNumber(); i++) {
			...
			if (player == null) {
				((OwnedCell) cell).setAvailable(true);
				cell.setNumHouses(0);
			} else {
	...
	public String[] getMonopolies() {
		...
		while (colors.hasMoreElements()) {
			...
			if (!(color.equals(RailRoadCell.COLOR_GROUP)) &&
					!(color.equals(UtilityCell.COLOR_GROUP))) {
	...
	public void purchase() {
		if (getPosition().isAvailable()) {
			...
			if (c instanceof PropertyCell) {
				purchaseProperty((PropertyCell) c);
			}
			if (c instanceof RailRoadCell) {
				purchaseRailRoad((RailRoadCell) c);
			}
			if (c instanceof UtilityCell) {
				purchaseUtility((UtilityCell) c);
			}
		}
	}
	...
	public void purchaseHouse(String selectedMonopoly, int houses) {
		...
		int totalCost = cells.length * cells[0].getHousePrice() * houses;
